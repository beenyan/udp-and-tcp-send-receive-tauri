...
const greetMsg = ref('');
const content = ref('');
const port = ref(socket.sctp.port);

async function send() {
    await invoke('sctp_send', { port: port.value, content: content.value })
        .then((message) => {
            console.log(message);
...
<template>
    <div class="card">
        ...
        <div class="btn-group">
            ...
            <button type="button" class="btn" @click="send()" :disabled="!socket.sctp.isListen">Send</button>
        </div>
    </div>
</template>
