...
const greetMsg = ref('');
const content = ref('');
const port = ref(socket.udp.port);

async function send() {
    await invoke('udp_send', { port: port.value, content: content.value })
        .then((message) => {
            console.log(message);
...
<template>
    <div class="card">
        ...
        <div class="btn-group">
            ...
            <button type="button" class="btn" @click="send()" :disabled="!socket.udp.isListen">Send</button>
        </div>
    </div>
</template>
